...
namespace Generic
{
    public class LocalXhtmlXmlResolver : XmlUrlResolver
    {
        ...
        public override object GetEntity(Uri absoluteUri, string role, Type ofObjectToReturn)
        {
            ...
            if(absoluteUri.Host=="xml.evernote.com"&&absoluteUri.AbsolutePath!="/pub/enml2.dtd")
            {
                switch(absoluteUri.LocalPath)
                {
                    case "/pub/-//W3C//ENTITIES Latin 1 for XHTML//EN":
                        return new FileStream(HttpRuntime.AppDomainAppPath + "DTD\\xhtml-lat1.ent", FileMode.Open);
                        
                    case "/pub/-//W3C//ENTITIES Symbols for XHTML//EN":
                        return new FileStream(HttpRuntime.AppDomainAppPath + "DTD\\xhtml-symbol.ent", FileMode.Open);
                        absoluteUri = new Uri("http://www.w3.org/TR/xhtml1/DTD/xhtml-symbol.ent"); break;
                    case "/pub/-//W3C//ENTITIES Special for XHTML//EN":
                        return new FileStream(HttpRuntime.AppDomainAppPath + "DTD\\xhtml-special.ent", FileMode.Open);
                        absoluteUri = new Uri("http://www.w3.org/TR/xhtml1/DTD/xhtml-special.ent"); break;
                    default: return new MemoryStream(); break;
                }
    ...
    {
        public static XDocument LoadWithLocalXhtmlXmlResolver(string strToLoad)
        {
            XDocument document = null;

            using (var stringReader = new StringReader(strToLoad))
            {
                var settings = new XmlReaderSettings
                {
                    ProhibitDtd = false,//,
                    XmlResolver = new LocalXhtmlXmlResolver(true)
