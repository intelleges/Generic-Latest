...
namespace Generic.Controllers
{
    ...
    public class PartnerController : Controller
    {
        ...
        protected void GenerateCreateDropDownLists()
        {
            ...
            try
            {
                ViewBag.LoggedInOwner = db.pr_getPerson(SessionSingleton.LoggedInUserId).FirstOrDefault().email;
                    ...
                    using (var context = new EntitiesDBContext())
                    {
                        try
                        {
                            ...
                            int? PartnerId = (int)context.pr_addPartnerSpreadsheetDataLoad(
                                ...
                                partners.dunsNumber,
                                partners.name,
                                partners.address1 != null ? partners.address1.Length > 255 ? partners.address1.Substring(0, 254) : partners.address1 : null,
                                partners.address2 != null ? partners.address2 : null,
                                partners.city,
                                stateIdSpreadSheet,
                                partners.zipcode != null ? partners.zipcode.Length > 200 ? partners.zipcode.Substring(0, 199) : partners.zipcode : null,
                                countryIdSpreadsheet,
                                partners.firstName,
                                partners.lastName,
                                partners.title,
                                partners.phone != null ? partners.phone.Length > 100 ? partners.phone.Substring(0, 199) : partners.phone : null,
                                partners.email,
        ...
        public ActionResult FindPartnerResult()
        {
            try
            {
                ...
                if (SessionSingleton.GridLevelMenuItems.Count == 0)
                {
                    ...
                    {
                        var menuItemDescription = db.pr_getGridLevelMenu(menuItemId).FirstOrDefault()?.description;
                        contextMenuItems.Add(menuItemDescription);
                        //SessionSingleton.GridLevelMenuItems.Add(menuItemDescription);
                    }
        ...
        public ActionResult SendRemind(int pptqId, int? statusId, int? touchpointId, int? partnerTypeId)
        {
            ...
            var reminder = db.pr_getReminder(Generic.Helpers.CurrentInstance.EnterpriseID, touchpointId, partnerTypeId, statusId, 1, pptqId).FirstOrDefault();
            string text = "";
            string subject = ""; string message = "";
            if (reminder != null)
            {
                var autoemail = db.pr_getAutomailMessage(reminder.automailToSend).FirstOrDefault();
                ...
                if (pptq != null && pptq.status == (int)PartnerStatus.Confirmed)
                {
                    pptq.invitedDate = DateTime.Now;
                    var person = db.pr_getPersonByEmail(CurrentInstance.EnterpriseID, User.Identity.Name).FirstOrDefault();
                    pptq.invitedBy = person.id;
        ...
        public ActionResult SendEmailByAccessCode(string accessCode, string subject, string text, HttpPostedFileBase attachment, int? autoMailId, bool? ccSender, string footer2)
        {
            ...
            var resultBody = formatter.sGetEmailBody(text, currentPerson, pptq.partner1, pptq.partnerTypeTouchpointQuestionnaire1.partnerType1.enterprise1, pptq.partnerTypeTouchpointQuestionnaire1.touchpoint1, pptq.partnerTypeTouchpointQuestionnaire1.id);

            subject = formatter.sGetEmailBody(subject, currentPerson, pptq.partner1, pptq.partnerTypeTouchpointQuestionnaire1.partnerType1.enterprise1, pptq.partnerTypeTouchpointQuestionnaire1.touchpoint1, pptq.partnerTypeTouchpointQuestionnaire1.id);
