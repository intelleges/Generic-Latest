...
namespace Generic.Helpers
{
    public static class EmailHelper
    {
           ...
           UrlHelper urlHelper, string requestUrl, person person)
        {
            ...
            var yesUrl = QuestionnaireHelper.GetQuestionnaireUrl(baseUri, urlHelper, partner.id, emailTo.First(), pptqObj.id, qnextId, 74);
            var noUrl = QuestionnaireHelper.GetQuestionnaireUrl(baseUri, urlHelper, partner.id, emailTo.First(), pptqObj.id, qnextId, 75);
            var unlockUrl = QuestionnaireHelper.GetQuestionnaireUrl(baseUri, urlHelper, partner.id, emailTo.First(), pptqObj.id, qnextId, -1);
            ...
            {
                string e = "";
                enterpriseSystemInfo objEnterpriseSystemInfo = db.pr_getEnterpriseSystemInfoAll().Where(o => o.enterprise == pptqObj.partnerTypeTouchpointQuestionnaire1.partnerType1.enterprise1.id).FirstOrDefault();
                if (objEnterpriseSystemInfo != null)
                    e = objEnterpriseSystemInfo.coordinatorEmail;

                str = t + "<br/><br/>EmailTo: " + emailTo + "<br/><br/>EmailFrom: " + e;
                SendEmail objSendEmail = new SendEmail();
                objSendEmail.sendEmail(mail, new EmailFormatSettings()
                {
                    sender = null,
        ...
        public static string SendEmailAlertWhere(partner partner, partnerPartnertypeTouchpointQuestionnaire pptqObj, string accessCode, string emailTo, int ptqId, int questionId, string qnextId, string text, Uri baseUri,
            UrlHelper urlHelper,string requestUrl, string ccEmail = "")
        {
            ...
            if (person != null)
            {
                str = t + "<br/><br/>EmailTo: " + emailTo + "<br/><br/>EmailFrom: "+ person.email;
                SendEmail objSendEmail = new SendEmail();
                objSendEmail.sendEmail(mail, new EmailFormatSettings()
                {
                    sender = null,
