...
namespace Generic.Helpers
{
    ...
    public class AsyncEvernoteAuthorizer : OAuthAuthorizer
    {
        ...
        {
            var result = await GetRequestToken(OAuthUrl, new Dictionary<string, string> { { "oauth_callback", callbackUri } });
            return result.Token;
        }
        ...
        /// </summary>
        /// <param name="token">The end point you plan on using to call ParseAccessToken</param>
        /// <returns></returns>
        public string BuildAuthorizeUrl(RequestToken token)
        {
            // Use the existing token, or generate a new one
            var callForwardUrl = BuildAuthorizeUrl(OAuthActionUrl, token);
