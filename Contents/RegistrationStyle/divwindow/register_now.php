...
if (webinar == 0) {
msg = msg + "Please select a webinar \n";
}
...
function getWebinar()
{
  ...
  $sqlquery="SELECT * FROM webinar";

  $results= mssql_query($sqlquery);

  while ($row=mssql_fetch_array($results)){
  $name = $name."<option value=".$row[id].">".$row[description]."</option><br/>";
  }
...
function getState()
{
  ...
  while ($row=mssql_fetch_array($results)){
  $name = $name."<option value=".$row[id].">".$row[stateCode]."</option><br/>";
  }
  mssql_close($sqlconnect);
  echo $name;
}
...
function getCountry()
{
  ...
  while ($row=mssql_fetch_array($results)){
  $name = $name."<option value=".$row[id].">".$row[name]."</option><br/>";
  }
  mssql_close($sqlconnect);
  echo $name;
}
...
<body>
<form name="contactUsForm" id="contactUsForm" method="post" action="doRegister.asp" >
...
<tr>
        <td align="right">Webinar*</td>
        <td></td>
        <td>
        <select name="webinar" id="webinar" class="contactBox">
      ...
      <tr>
        ...
        <td></td>
        <td><select name="state" id="state" style="width: 260px;">
		<option value = "0">Please select one</option>
		<?php getState(); ?>
          </select>
        </td>
      ...
      <tr>
        ...
        <td></td>
        <td><select name="country" id="country" style="width: 260px;">
		<option value = "0">Please select one</option>
	<?php getCountry() ?>
        </select></td>
      </tr>
      ...
      <tr>
        ...
        <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
            <tr>
              ...
              <td width="72%"><input class="contactcancle" type="button" name="submit2" id="submit2" value="" /></td>
            </tr>
        </table></td>
      </tr>
      <tr>
